...
Nothing here decides *whether* to colorize. `click.echo` strips ANSI when stdout
...
_MARKS = {
    "OK": lambda s: click.style(s, fg="green", bold=True),
    "DRY": lambda s: click.style(s, fg="cyan", bold=True),
    "SKIP": lambda s: click.style(s, fg="yellow", bold=True),
    "FAIL": lambda s: click.style(s, fg="red", bold=True),
}
...
def verdict(good: bool, s: str) -> str:
    ...
    return click.style(s, fg="green" if good else "red", bold=True)
...
#
# force_terminal puts the codes into the string unconditionally and click decides
